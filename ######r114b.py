...
class DistilledR1Engine:
    ...
    # -------------------------------------------------------------------------
    # Build reasoning steps in R1 style
    # -------------------------------------------------------------------------
    def _build_reasoning_steps(self, query: str) -> List[str]:
        """Return a list of natural language reasoning steps."""
        steps = []
        q_lower = query.lower().strip()

        # 1. Initial parsing
        steps.append("Okay, let me think about this step by step.")
        steps.append(f"The user says: \"{query}\"")

        # 2. Categorize the query
        if any(greet in q_lower for greet in ["hello", "hi", "hey", "greetings"]):
            steps.append("This seems like a greeting. I should respond warmly and offer assistance.")
            steps.append("I'll keep it friendly and open‑ended.")
        elif "how are you" in q_lower:
            steps.append("They're asking about my state. I can mention my 4‑bit quantized nature and that I'm functioning well.")
            steps.append("Maybe add a touch of personality.")
        elif "your name" in q_lower:
            steps.append("They want to know who I am. I'll introduce myself with the model specs.")
            steps.append("It's good to include the distillation and quantization details.")
        elif "?" in q_lower:
            steps.append("This is a question. I need to search my internal knowledge base.")
            steps.append("Let me break it down into key parts.")
            if "why" in q_lower or "how" in q_lower:
                steps.append("It's a 'why' or 'how' question – I should provide an explanation, not just a fact.")
            steps.append("I'll consider possible answers and check for consistency.")
        else:
            steps.append("This appears to be a statement or general comment. I should acknowledge it and encourage further conversation.")

        # 3. Incorporate model‑specific knowledge (quantization, distillation)
        steps.append(f"Remember: I'm a 14B parameter model quantized to 4 bits, which gives me a small footprint ({MODEL_SIZE_GB:.2f} GB).")
        steps.append("Thanks to R1 distillation, I can generate fluent English despite the low‑bit weights.")

        # 4. Deeper reasoning if DeepThink is enabled
        if self.deep_mode:
            steps.append("DeepThink engaged: I'll double‑check my reasoning and consider edge cases.")
            steps.append("Let me simulate a few transformer layers mentally...")
            steps.append("The 4‑bit quantization introduces some noise, but distillation helps maintain accuracy.")
            steps.append("I should also verify that my answer aligns with common knowledge.")
            steps.append("Wait, is there any ambiguity in the query? Let me re‑read it carefully.")
            steps.append("Hmm, maybe I missed a nuance. Let me think again.")
            steps.append("Alright, I think I've considered the main points.")

        # 5. Wrapping up the reasoning
        steps.append("I think I have enough context to formulate a helpful response.")
        steps.append("Now I'll generate the answer using my distilled language capabilities.")
    ...
    def generate_thought_chain(self, query: str, message_queue: queue.Queue):
        hidden = self._generate_vector(query)  # still computed for realism

        # Run through first and last block for representativeness (optional)
        ...
        # Optionally add activation values for technical interest
        if random.random() < 0.3:
            stream_thought(f"(Layer 0 activation: {act_0:.4f}, Layer {NUM_LAYERS-1} activation: {act_n:.4f})")
    ...
    def _generate_response(self, query: str, message_queue: queue.Queue):
        ...
        if any(greet in q_lower for greet in ["hello", "hi", "hey", "greetings"]):
            base = "Hello! I'm R1Cat, a 14B parameter 4‑bit quantized model. How can I help you today? Feel free to ask me anything!"
        elif "how are you" in q_lower:
            base = "I'm doing great, thanks for asking! My 4‑bit weights are stable and my distilled language capabilities are fully active. How about you?"
        elif "your name" in q_lower:
            base = "I'm called R1Cat-Distill-4Bit-14B – a simulation of a 14B transformer quantized to 4 bits, with R1's distillation for fluent English. You can just call me R1Cat!"
        elif "?" in q_lower:
            # For questions, give a thoughtful answer
            base = (f"That's an interesting question. After walking through my reasoning, I'd say that "
                    f"the answer depends on several factors. In general, {self._generate_generic_answer(q_lower)} "
                    f"Would you like me to elaborate on any specific aspect?")
        else:
            # Default conversational response
            base = (f"I hear you saying: \"{query}\". My reasoning suggests that you might be interested in "
                    f"exploring this topic further. I'm here to assist – just let me know what you'd like to discuss!")

        # Optional addition about quantization savings
        if random.random() < 0.5:
            savings = (1 - MODEL_SIZE_GB / (SIMULATED_TOTAL * 2 / 1024**3)) * 100
            base += f" (By the way, my 4‑bit quantization saves about {savings:.0f}% memory compared to fp16.)"
    ...
    def _generate_generic_answer(self, query: str) -> str:
        ...
        if "what" in query:
            return "it's a concept that can be understood from multiple perspectives."
        elif "why" in query:
            return "the reasons often involve historical context and practical considerations."
        elif "how" in query:
            return "the process typically involves several steps that need to be followed carefully."
        else:
            return "there isn't a one‑size‑fits‑all answer, but I can guide you through the key points."
